...
```bash
export IoT_PG_BOARDS='feather-stm32f405,nucleo-n657x0:net'
mise run build    # builds both, nucleo-n657x0 with the net project
mise run flash    # targets the first entry (feather-stm32f405)
```

`IoT_PG_BOARDS` takes comma-separated `board[:project]` entries; a listed
...
topic filters override, and `IoT_PG_TOPICS` (comma-separated filters)
